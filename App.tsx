...
type ViewState = 'search' | 'login' | 'dashboard' | 'admin' | 'terms';

export default function App() {
  const [currentView, setCurrentView] = useState<ViewState>(() => {
    const savedView = localStorage.getItem('sad_view');
    return (savedView as ViewState) || 'search';
  });
  const [user, setUser] = useState<User | null>(() => {
    const savedUser = localStorage.getItem('sad_user');
    return savedUser ? JSON.parse(savedUser) : null;
  });

  // Persist state changes
  React.useEffect(() => {
    localStorage.setItem('sad_view', currentView);
  }, [currentView]);
  ...
  const handleSearch = useCallback(async (searchQuery: string = query) => {
    ...
    try {
      ...
      // If no exact results found, try searching for alternatives
      if (data.length === 0) {
...
=======
            <button onClick={() => setCurrentView('terms')} className="hover:text-primary-600 transition-colors">شروط الاستخدام</button>
            <button onClick={() => setCurrentView('terms')} className="hover:text-primary-600 transition-colors">سياسة الخصوصية</button>
            <button onClick={() => setCurrentView('terms')} className="hover:text-primary-600 transition-colors">إخلاء المسؤولية</button>
>>>>>>> b75c855c49f0bf120451948a9c5fc2083f2a4ddd
          </div>
